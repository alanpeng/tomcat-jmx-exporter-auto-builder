...
# Add a line
sed -i "$line r tomcat-jmx-config-string.txt" DockerBuild-$TIME_STRING/catalina.sh
# Replace the port number
sed -i 's#$JMX_PORT#'"$JMX_AGENT_PORT"'#g' DockerBuild-$TIME_STRING/catalina.sh
...
sed -i 's#latest#'"$TOMCAT_VERSION_TAG"'#g' DockerBuild-$TIME_STRING/Dockerfile
